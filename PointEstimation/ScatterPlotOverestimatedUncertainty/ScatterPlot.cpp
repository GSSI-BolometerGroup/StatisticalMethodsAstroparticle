...
int main()
{
    // We will generate N points distributed along a linear curve f(x)=a+bx
    ...
    // We will assign an artificially overestimated uncertainty to the points,
    // and fit the scatter plot with a straight line g(x)=a+bx.
    // In other words:
    // x_i       = equally spaced points
    // sigma_i   = flat random number in [10,100] used to smear the points around the curve
    // dy_i      = Gaussian random number with mean 0 and sigma=sigma_i
    // y_i       = f(x_i) + dy_i
    // To each point y_i, we assign the uncertainty k*sigma_i, with k>1.
    ...
    // We repeat this a few times for different values of k.
    ...
    int nBinsChi2 = 100;
    std::vector<TH1D*> h_Chi2(nK);
    for( size_t k=0; k<nK; k++ )
	{
	    std::string name( "k=" + std::to_string(K[k]) );
	    h_Chi2[k] = new TH1D(name.c_str(),name.c_str(),nBinsChi2,0,maxChi2);
	    h_Chi2[k]->GetXaxis()->SetTitle("#Chi^{2}");
	}
    ...
    h_Chi2[4]->SetLineColor(kMagenta+2);
